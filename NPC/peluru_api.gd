extends Area2D


var arah2 
var speed = 300
var velocity
var power
var npcID

func _physics_process(delta):
	set_global_position(get_global_position() + velocity * delta)
	pass


func _tembak(arah):
	if arah == Vector2(1, 0):
		$AnimationPlayer.play("tembak_idle_kanan")
	if arah == Vector2(-1, 0):
		$AnimationPlayer.play("tembak_idle_kiri")
	if arah == Vector2(0, 1):
		$AnimationPlayer.play("tembak_idle_bawah")
	if arah == Vector2(0, -1):
		$AnimationPlayer.play("tembak_idle_atas")
	velocity = arah * speed
	arah2 = arah


func _on_peluru_api_body_entered(body):
	
	if !body.is_in_group("musuh"):
		$AnimationPlayer.play("tembak_kena")
		velocity = Vector2.ZERO
	
		if body.is_in_group("player"):
			Global.npcDmg[npcID]["player"] += power 
			body.emit_signal("player_hit", power)
		
		elif body.is_in_group("tower") and body.state == "aman":
			Global.npcDmg[npcID]["tower"] += power
			body.emit_signal("tower_hit", power, body.state)
		
		elif body.is_in_group("main_tower") and body.state == "aman" and body.can_hit:
			Global.npcDmg[npcID]["tower"] += power
			body.emit_signal("maintower_hit", power, body.state)
		
		elif body.is_in_group("house") and body.state == "aman":
			Global.npcDmg[npcID]["rumah"] += power
			body.emit_signal("house_hit", power, body.state)
